...
export function HomeView({ games, lists }: { games: Game[]; reviews: Review[]; lists: CuratedList[] }) {
  ...
  const trendingGames = games.filter((game) => game.trending).slice(0, 10);
      ...
      <section style={{ position: 'relative', minHeight: '92vh', display: 'flex', alignItems: 'center', overflow: 'hidden', padding: '0 24px' }}>
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto', width: '100%', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 40, alignItems: 'center', position: 'relative', zIndex: 1 }}>
          ...
          <div style={{ position: 'relative', height: 520 }}>
            {collageGames.map((game, i) => <Link key={game.id} href={`/games/${game.id}`} style={{ position: 'absolute', top: `${(i % 3) * 24 + 4}%`, left: `${i * 18 % 70}%`, transform: `rotate(${[-8, 5, -3, 11, -6][i] ?? 0}deg)`, zIndex: i + 1 }}><img src={game.coverImage} alt={game.title} style={{ width: 120 + i * 6, height: 180 + i * 9, objectFit: 'cover', borderRadius: 12, boxShadow: '0 20px 60px rgba(0,0,0,0.7)', border: '2px solid rgba(255,255,255,0.08)' }} /></Link>)}
          </div>
        </div>
      </section>
      <section style={{ padding: '64px 24px', maxWidth: 1200, margin: '0 auto' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 28 }}><h2 style={{ margin: 0, fontFamily: 'Space Grotesk, sans-serif', fontSize: '1.4rem', fontWeight: 700, color: '#F0F0F5' }}>Trending Games</h2><div style={{ display: 'flex', gap: 8 }}><button onClick={() => scrollRef.current?.scrollBy({ left: -300, behavior: 'smooth' })} style={iconButton}><ChevronLeft size={16} /></button><button onClick={() => scrollRef.current?.scrollBy({ left: 300, behavior: 'smooth' })} style={iconButton}><ChevronRight size={16} /></button></div></div>
        <div ref={scrollRef} style={{ display: 'flex', gap: 16, overflowX: 'auto', paddingBottom: 8 }}>{trendingGames.map((game) => <div key={game.id} style={{ flexShrink: 0, width: 200 }}><GameCard game={game} /></div>)}</div>
      </section>
      <section style={{ padding: '64px 24px', background: 'var(--gl-bg-surface)', borderTop: '1px solid var(--gl-border)', borderBottom: '1px solid var(--gl-border)' }}><div style={{ maxWidth: 1200, margin: '0 auto' }}><div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 28 }}><List size={18} color="#39FF85" /><h2 style={{ margin: 0, fontFamily: 'Space Grotesk, sans-serif', fontSize: '1.4rem', color: '#F0F0F5' }}>Top Curated Lists</h2></div><div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 20 }}>{lists.slice(0, 3).map((list) => <ListCard key={list.id} list={list} games={games} />)}</div></div></section>
